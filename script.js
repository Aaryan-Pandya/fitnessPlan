...
function getSelectedStyles() {
  return plannerEls.styleChecks.filter((box) => box.checked).map((box) => box.value);
}
...
function syncConditionalSteps() {
  ...
  if (!getSelectedStyles().length && plannerEls.styleChecks.length) {
    plannerEls.styleChecks[0].checked = true;
  }
}
...
function renderPlannerStep() {
  ...
  if (plannerEls.stepProgress) {
    const progress = ((plannerStepIndex + 1) / visibleSteps.length) * 100;
    plannerEls.stepProgress.style.width = `${progress}%`;
  }

  if (plannerEls.backBtn) {
  ...
  plannerEls.saveBtn?.classList.toggle("hidden", currentGeneratedPlan == null);
}
...
function initPlanner() {
  ...
  plannerEls.styleChecks.forEach((box) => box.addEventListener("change", renderPlannerStep));
...
function validatePlannerContext(ctx) {
  ...
  if (!ctx.mileTime || parseTimeToSeconds(ctx.mileTime) == null) return "Enter a real mile time like 8:05.";
  if (ctx.runDurationMinutes < 5) return "Longest continuous run should be at least 5 minutes.";
  if (ctx.runDistanceValue <= 0) return "Longest run distance should be greater than 0.";
  return "";
}

function getWorkoutTemplates(daysPerWeek) {
  if (daysPerWeek === 3) return ["strengthA", "runQuality", "strengthB"];
  if (daysPerWeek === 4) return ["strengthA", "runQuality", "strengthB", "runEasy"];
...
function makeRunExercise(ctx, weekIndex, id, easy = false, test = false) {
  ...
  const minutes = easy
    ? Math.max(12, Math.round(ctx.runDurationMinutes * (1 + (weekIndex - 1) * 0.08)))
    : ctx.primaryGoal === "mile"
      ? Math.max(10, ctx.runDurationMinutes - 2 + weekIndex)
      : Math.max(12, ctx.runDurationMinutes + weekIndex);

  return makeExercise(id, easy ? "Easy Run" : ctx.primaryGoal === "mile" ? "Quality Run" : "Tempo Run", {
    sets: 1,
    start: minutes,
    max: minutes,
    step: 0,
    targetType: "minutes",
    tempo: easy ? "Easy conversational effort" : ctx.primaryGoal === "mile" ? "Strong repeatable effort" : "Controlled hard effort",
    rest: "—",
    progressionType: "static",
    loadLabel: "Distance"
  });
...
function buildPlan(ctx) {
  ...
  const summaryParts = [
    ...
    ctx.primaryGoal === "mile" ? "Mile focus" : ctx.primaryGoal === "mixed" ? "Running + strength" : "5K focus",
    ctx.styles.join(", "),
    `${ctx.runDurationValue} ${ctx.runDurationUnit} longest continuous run`,
    `${ctx.runDistanceValue} ${ctx.runDistanceUnit} longest run distance`
  ];
...
function renderLoggedOutState(user = null) {
  if (dashEls.profileBox) {
    dashEls.profileBox.innerHTML = `<div class="empty-box">${user ? "No plan found yet." : "Please log in to view your dashboard."}</div>`;
  }
  if (dashEls.currentPlanBox) dashEls.currentPlanBox.innerHTML = `<div class="empty-box">No plan loaded.</div>`;
  ...
  if (dashEls.weekTrackerBox) dashEls.weekTrackerBox.innerHTML = `<div class="empty-box">No week loaded.</div>`;
  if (dashEls.calendarBox) dashEls.calendarBox.innerHTML = `<div class="empty-box">No future weeks loaded.</div>`;
  if (dashEls.streakHeroNumber) dashEls.streakHeroNumber.textContent = "0";
  if (dashEls.streakHeroSub) dashEls.streakHeroSub.textContent = "Log workouts to build your streak.";
}
...
function renderCurrentPlan(plan) {
  ...
  dashEls.currentPlanBox.innerHTML = `
    ...
    <div class="info-card">
      ...
      <div class="info-value">${escapeHTML(plan.primaryGoal === "mile" ? "Mile focus" : plan.primaryGoal === "mixed" ? "Running + strength" : "5K focus")}</div>
    </div>
    <div class="info-card">
...
function renderStreak(plan, tracker) {
  ...
  if (dashEls.streakHeroSub) {
    if (streak === 0) dashEls.streakHeroSub.textContent = "No streak yet. Log today’s session to start one.";
...
function renderToday(plan, tracker) {
  ...
  (displayWorkout.exercises || []).forEach((exercise) => {
    ...
    if (exercise.targetType === "minutes") {
      exerciseList.appendChild(createTodayRunBox(workout, tracker, exercise));
    } else {
...
function createTodayRunBox(workout, tracker, exercise) {
  ...
  box.innerHTML = `
    ...
    <label>${isMile ? "Log time (m:ss)" : `Log distance in ${getLatestPlan()?.runDistanceUnit || "miles"}`}</label>
