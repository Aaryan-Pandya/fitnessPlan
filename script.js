...
function getMetricForEntry(exercise, entry) {
  ...
  if (category === "run-test" || (category === "run" && name.includes("mile"))) {
    ...
    if (secs != null) return { value: secs, better: "lower" };
  }

  if (category === "run") {
    const dist = parseFirstNumber(entry.logText);
    if (dist != null) return { value: dist, better: "higher" };
    return null;
  }
  ...
  if (
    ...
    return { value: Number(entry.loadValue), better: "lower" };
  }
...
function buildNormalRunDay(title, weekIndex, ctx) {
  ...
  const easyMinutes = Math.round(ctx.runBlockMinutes * Math.pow(youthMultiplier, weekIndex));
...
function applyDeloadToBuiltDay(day) {
...
function buildWeekSchedule(weekIndex, weekStart, ctx, signals) {
  ...
  for (let dayOffset = 0; dayOffset < 7; dayOffset += 1) {
    ...
    if (weekType === "deload") {
      built = applyDeloadToBuiltDay(built);
    }
...
function getContextFromForm() {
  const user = getUser();
  const age = toNumber(els.age.value || user?.age, 13);
  ...
  return {
    ...
    pushUpMax: toNumber(els.pushUpMax.value, 10),
    plankMax: toNumber(els.plankMax.value, 45),
...
function initializePlanner() {
  ...
  if (!els.pushUpMax.value) els.pushUpMax.value = "10";
  if (!els.plankMax.value) els.plankMax.value = "45";
  if (!els.runBlockMinutes.value) els.runBlockMinutes.value = "10";
  if (!els.runBlockDistance.value) els.runBlockDistance.value = "0.8";
  ...
  if (existingPlan?.schedule?.length) {
    ...
    setStatus("Loaded your latest saved plan preview.");
  } else {
