...
function collectBands() {
  ...
  if (pushupAbility === "No") upperBands.push(1);
...
function validateBaselines() {
  ...
  if (showRun) {
    if (Number.isNaN(parseTimeToSeconds(document.getElementById("mileTime").value))) return "Enter mile time in mm:ss.";
    if (Number.isNaN(parseTimeToSeconds(document.getElementById("run400Time").value))) return "Enter 400m time in mm:ss.";
  }
  ...
  if (showCore && !document.getElementById("plankSeconds").value) return "Enter plank hold in seconds.";
  if (showEndurance && !document.getElementById("longestContinuousMinutes").value) return "Enter your longest continuous cardio session in minutes.";
  if (showFlexibility && !document.getElementById("toeTouch").value) return "Choose whether you can touch your toes.";
...
function buildStrengthTemplates(bands) {
  ...
  if (includePower) {
    templateA.mainWork.push(`Low-volume jump or fast step-up — 3 sets of 4-6 reps. Rest 75 sec.`);
  }
